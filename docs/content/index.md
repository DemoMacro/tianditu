...
seo:
  title: 天地图工具库
  description: 天地图 JavaScript API 与 REST 服务的类型化封装，提供 Vue 3 组件、React 组件、Web Components 与服务客户端。
---
...
把天地图装进你的项目
#description
天地图 JavaScript API 与 REST 服务的类型化封装。地图可以写成 Vue 3 组件、React 组件或原生 Web Components；搜索、路线规划等服务通过客户端函数调用。在右侧输入密钥，地图即可运行。
#links
:::u-button
---
label: 快速开始
...
features:
  ...
  - icon: i-lucide-monitor-smartphone
    ...
    description: 同一套核心。Vue 项目用 `@tianditu/vue`，React 项目用 `@tianditu/react`，其他环境用 `@tianditu/web-components`。
...
#description
Vue 组件、React 组件、原生自定义元素、独立服务客户端，共享同一个核心，按项目需要选用。
#links
:::u-button
---
color: neutral
