...
```tsx [React]
import { TdtMap } from "@tianditu/react";
...
React 版事件名为驼峰化的 `onXxx` 回调：

```tsx
<TdtMap tk="…" onClick={(e) => console.log(e.lnglat)} />
```

::note
上表之外的地图事件（如 `mousemove`、`dblclick`、`zoomstart`）不经模板直发，需要时用 `useMap` 拿到实例后 `map.addEventListener` 命令式挂接。
::
...
```tsx [React]
import { useMap } from "@tianditu/react";

const { map, ready } = useMap();

function flyTo() {
  map?.centerAndZoom(new T.LngLat(116.404, 39.915), 14);
}
```

:::

模板引用同样可用：`<TdtMap ref="mapRef">`（Vue）或 `<TdtMap ref={mapRef}>`（React）的句柄上都有 `map`、`session`、`ready`。
