...
:::DemoMap
<tdt-marker :lnglat="[116.404, 39.915]" draggable title="天安门" />
:::
...
```tsx [React]
<TdtPolyline path={[[116.38, 39.9], [116.42, 39.92]]} color="#2563eb" weight={4} />
<TdtPolygon path={[[116.35, 39.86], [116.38, 39.86], [116.38, 39.88]]} fillColor="#16a34a" fillOpacity={0.35} />
```
...
<TdtRectangle bounds={[[116.33, 39.94], [116.35, 39.96]]} fillColor="#ca8a04" />
```
...
```tsx [React]
<TdtMarkerCluster gridSize={60}>
  {markers.map((m) => (
    <TdtMarker key={m.name} lnglat={m.lnglat} />
  ))}
</TdtMarkerCluster>
```
...
```tsx [React]
<TdtContextMenu>
  <TdtContextMenuItem text="居中到此点" onSelect={(lnglat) => …} />
</TdtContextMenu>
```
